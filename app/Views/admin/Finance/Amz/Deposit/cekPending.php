        ...
        <tbody>
            <?php foreach($dataUpload as $row): ?>
                <tr>
                    ...
                    <td><?=$row['destination_account']?></td>
                    <td><?=$row['reply']?></td>
                    <td><img src="<?=getenv('API_HOST')."/api/finance/a/image/".$row['id']?>" width="100" alt=""></td>
                    <td>
                        <a href="<?=base_url('/finance/deposit/amz/add_reply/'.$row['id'])?>" class="btn btn-success text-white">Reply Provider</a>
                    </td>
                </tr>
            <?php endforeach ?>
        </tbody>
